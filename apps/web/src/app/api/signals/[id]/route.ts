...
import { UpdateSignalSchema } from '@yggdrasight/core'
...
export async function GET(_req: NextRequest, context: RouteContext) {
  ...
  return withAuth(async (ctx) => {
    try {
      const signal = await ctx.models.Signal.findById(id)
      if (!signal) {
...
export async function PATCH(req: NextRequest, context: RouteContext) {
  ...
  return withAuth(async (ctx) => {
    try {
      ...
      const signal = await ctx.models.Signal.findByIdAndUpdate(id, update, { new: true })
      if (!signal) {
        return NextResponse.json({ error: 'Signal not found' }, { status: 404 })
      }
...
export async function DELETE(_req: NextRequest, context: RouteContext) {
  ...
  return withAuth(async (ctx) => {
    try {
      const signal = await ctx.models.Signal.findByIdAndDelete(id)
      if (!signal) {
        return NextResponse.json({ error: 'Signal not found' }, { status: 404 })
      }
